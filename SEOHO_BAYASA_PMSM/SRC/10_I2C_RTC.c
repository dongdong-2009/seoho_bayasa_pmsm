...
void Set_InitialTime(void)
{
	...
	Write_RTC_Data(RTC_REGS_SECOND, 0x15);		// CH bit=0(enable oscillator), second
	Write_RTC_Data(RTC_REGS_MINUTE, 0x49);		// minute
	Write_RTC_Data(RTC_REGS_HOUR, 	0x15);		// hour
	Write_RTC_Data(RTC_REGS_WEEK, 	0x01);		// week
	Write_RTC_Data(RTC_REGS_DATE, 	0x12);		// date
	Write_RTC_Data(RTC_REGS_MONTH, 	0x06);		// month
